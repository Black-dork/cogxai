...
### Added
- **Hosted Cortex mode** — Zero-setup memory via API key (`npx cogxai register`)
...
- **CLI register command** — `npx cogxai register` for hosted mode onboarding
- **CLI init wizard** — Hosted vs self-hosted mode selection in `npx cogxai init`
...
- Block CogxAI from engaging specific problematic threads

## [2.4.0] — 2026-02-15

### Changed
- Slimmed npm package to pure memory SDK — bot internals no longer shipped
- Removed unused `@base/spl-token` dependency

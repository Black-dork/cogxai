---
name: CogxAI Memory MCP
description: MCP server for CogxAI's 4-tier cognitive memory system — store, recall, search, and dream. Built on Supabase + pgvector with type-specific decay, Hebbian association graphs, and on-chain commitment via Base.
metadata:
  openclaw:
    requires:
      env:
...
### `store_memory`
Store a new memory. Memories persist across conversations, decay over time if not accessed, and get committed to Base.
...
### `ask_cogxai`
Ask CogxAI a question and get an in-character response. Calls Claude API.
...
- **On-chain commitment**: SHA-256 hashed memories committed to Base via memo transactions

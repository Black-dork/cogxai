/**
 * COGXAI — Persistent memory for AI agents.
 * Local-first, zero config.
 *
 * @example
 * ```typescript
 * import { remember, recall } from 'cogxai'
 *
 ...
 * 
 * @example
 * import { init, remember, recall } from 'cogxai'
 * init({ wallet: '5vK6WRCq...BGv2r', name: 'CogxAI' })
 */
 ...
 * The pack is self-contained and can be imported by any CogxAI agent.
 * 
 * @example
 * const pack = exportPack()
 * fs.writeFileSync('brain.cogxai.json', JSON.stringify(pack))
 */
 ...
 * 
 * @example
 * const pack = JSON.parse(fs.readFileSync('brain.cogxai.json', 'utf-8'))

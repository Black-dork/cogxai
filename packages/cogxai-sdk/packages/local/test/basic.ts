...
async function main() {
  console.log('🧠 COGXAI Local — Full Test (gte-small)\n');

  const store = createStore('/tmp/cogxai-test-v2/memory.db');
  ...
  const memories = [
    ...
    { content: "COGXAI uses four memory types: episodic for raw events, semantic for knowledge, procedural for learned behaviors, and self_model for identity", type: 'semantic' as const, importance: 1.0 },
    ...
    { content: "Venice AI provides private inference. COGXAI uses Venice for embedding and LLM calls to ensure user data stays private", type: 'semantic' as const, importance: 0.8 },
  ...
  const queries = [
    ...
    { q: "what deployment modes are there?", expect: "local" },

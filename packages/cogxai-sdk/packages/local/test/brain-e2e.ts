/**
 * End-to-end test: @cogxai/brain engine + SQLite provider + gte-small embeddings.
 * Tests the full stack from brain → local.
 */

// Import brain engine from workspace (would be `@cogxai/brain` in npm)
import { CogxAIEngine } from '../../cogxai-core/src/engine.js';
import { SQLiteProvider } from '../src/sqlite-provider.js';
import { GteSmallEmbeddings } from '../src/gte-embeddings.js';
import { rmSync, existsSync } from 'fs';

const DB_PATH = '/tmp/cogxai-brain-e2e/memory.db';
let pass = 0, fail = 0;
...
async function main() {
  // Clean slate
  if (existsSync('/tmp/cogxai-brain-e2e')) rmSync('/tmp/cogxai-brain-e2e', { recursive: true });

  const storage = new SQLiteProvider({ dbPath: DB_PATH, wallet: '5vK6WRCq5V6BCte8cQvaNeNv2KzErCfGzeBDwtBGGv2r', name: 'CogxAI' });
  const embeddings = new GteSmallEmbeddings();

  console.log('\n🧠 @cogxai/brain + SQLite + gte-small — Full Stack E2E\n');
  ...
  assert(m1.id.startsWith('cogxai-'), `Hash ID: ${m1.id}`);
  ...
  await engine.store({ content: 'CogxAI launched in March 2026 as an open-source persistent memory layer for AI agents', type: 'episodic', importance: 0.8, source: 'conversation' });
  await engine.store({ content: 'COGXAI uses four memory types: episodic, semantic, procedural, self_model', type: 'semantic', importance: 0.9, source: 'knowledge-seed' });
  ...
  await engine.store({ content: 'The COGXAI token will launch on Base. Contract address will be announced on cogxai.org', type: 'semantic', importance: 0.7, source: 'conversation' });
  ...
  const queries = [
    ...
    { q: 'how does COGXAI memory work?', expect: 'four memory types' },
  ...
  const storage2 = new SQLiteProvider({ dbPath: '/tmp/cogxai-brain-e2e/agent-b.db', wallet: 'AgentB-wallet', name: 'Agent B' });
  ...
  const engine2 = new CogxAIEngine({ storage: storage2 as any, embeddings, query_expansion: false, entity_extraction: false, vector_threshold: 0.2 });
  await new Promise(r => setTimeout(r, 200));
  const imported = await engine2.recall({ query: 'COGXAI memory types', limit: 3 });

...
const PACK_PATH = '/tmp/cogxai-portability/brain.cogxai.json';

async function main() {
  console.log('🔗 COGXAI Portability Test — End-to-End Journey\n');

  // ── Step 1: Agent A builds memories ──────────────────────

  console.log('1️⃣  Agent A stores memories with wallet identity...');
  const agentA = createStore({
    dbPath: '/tmp/cogxai-portability/agent-a.db',
    wallet: WALLET_A,
    name: 'CogxAI',
  });
  agentA.clear();
  ...
  const m1 = await agentA.remember({
    content: 'CogxAI uses Base as a public memory ledger with 4 cognitive memory types',
    type: 'semantic',
    importance: 1.0,
  });
  ...
  const m3 = await agentA.remember({
    content: 'CogxAI launched in March 2026 as a persistent memory layer for AI agents',
    type: 'episodic',
    importance: 0.8,
  });
  ...
  const results = await agentA.recall({ query: 'how does memory work on Base?', limit: 3 });
  ...
  const agentB = createStore({
    dbPath: '/tmp/cogxai-portability/agent-b.db',
    wallet: WALLET_B,
    name: 'Agent B',
  });
  agentB.clear();
  ...
  const bResults = await agentB.recall({ query: 'Base memory types', limit: 3 });
  ...
  const m1imported = agentB.export().find(m => m.content.includes('Base as a public memory'));
  if (m1imported) {
  ...
  const mdPath = '/tmp/cogxai-portability/brain.md';

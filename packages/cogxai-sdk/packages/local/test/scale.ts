...
async function main() {
  console.log('🧠 COGXAI Local — Scale Test (100+ memories)\n');

  const store = createStore('/tmp/cogxai-scale/memory.db');
  store.clear();
  await store.warmup();
  ...
  const memories = [
    // User identity (10)
    { content: "User's name is Alex, goes by Al", type: 'semantic' as const, importance: 0.9 },
    { content: "Al is based in Berlin, timezone UTC+1", type: 'semantic' as const, importance: 0.7 },
    { content: "User previously worked at Acme Corp, resigned February 2026", type: 'episodic' as const, importance: 0.8 },
    { content: "User's email is alex@example.com", type: 'semantic' as const, importance: 0.6 },
    { content: "User's Telegram handle is @testuser", type: 'semantic' as const, importance: 0.5 },
    { content: "Al has a wife and family, prioritizes family time", type: 'semantic' as const, importance: 0.7 },
    ...
    { content: "User's X/Twitter handle is @testuser", type: 'semantic' as const, importance: 0.5 },

    // Technical project (15)
    { content: "CogxAI is a persistent memory system for AI agents built on Base", type: 'semantic' as const, importance: 1.0 },
    { content: "CogxAI uses four memory types: episodic, semantic, procedural, self_model", type: 'semantic' as const, importance: 0.9 },
    ...
    { content: "The COGXAI token contract address is ", type: 'semantic' as const, importance: 1.0 },
    ...
    // Events timeline (15)
    { content: "March 2026: CogxAI launched as open-source persistent memory for AI agents", type: 'episodic' as const, importance: 0.8 },
    { content: "FEiKU joined as marketing advisor in late February", type: 'episodic' as const, importance: 0.7 },
    { content: "Red Beard Ventures reached out for a VC call about investing in CogxAI", type: 'episodic' as const, importance: 0.8 },
    ...
    { content: "March 4: Registered CogxAI on 8004 Agent Registry on Base mainnet", type: 'episodic' as const, importance: 0.9 },
    ...
    { content: "Token launch application drafted, needs personal sections", type: 'episodic' as const, importance: 0.6 },
    { content: "gmoney raised concerns about privacy and cost at scale for CogxAI", type: 'episodic' as const, importance: 0.6 },
    ...
    { content: "10 Days of Building CogxAI campaign designed with 100M token rewards", type: 'episodic' as const, importance: 0.6 },
    ...
    { content: "Talked about the Base ecosystem growth and TVL numbers", type: 'episodic' as const, importance: 0.4 },
    ...
    { content: "Discussed the deployment architecture: Railway for backend, Supabase for database, Cloudflare for CDN", type: 'episodic' as const, importance: 0.7 },
    ...
    { content: "I am a CogxAI-powered memory agent with persistent recall across sessions", type: 'self_model' as const, importance: 0.9 },
    ...
    { content: "Run benchmarks with: cd cogxai-benchmark-v2 && python3 run_live.py", type: 'procedural' as const, importance: 0.6 },
  ...
  const queries = [
    // Identity
    { q: "what's the user's name?", expect: "Alex" },
    { q: "where does the user live?", expect: "Berlin" },
    { q: "what was the user's previous job?", expect: "Acme" },
    { q: "what's the user's email?", expect: "alex@example" },
    
    // Technical
    { q: "what embedding model does CogxAI use?", expect: "Voyage" },
    { q: "what database does CogxAI use?", expect: "Supabase" },
    { q: "token contract address?", expect: "Base" },
    { q: "how does memory consolidation work?", expect: "dream" },
    { q: "what are the four memory types?", expect: "episodic" },
    
    // Events
    { q: "when did CogxAI launch?", expect: "March 2026" },
    ...
    { q: "how does CogxAI compare to SuperMemory?", expect: "SuperMemory" },
  ];

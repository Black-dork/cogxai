...
async function main() {
  console.log('🧠 COGXAI Local — Scale + Edge Case Test\n');

  const store = createStore('/tmp/cogxai-stress/memory.db');
  store.clear();
  ...
  const memories = [
    ...
    // COGXAI architecture
    { content: "COGXAI uses four memory types: episodic for raw events, semantic for knowledge, procedural for behaviors, self_model for identity", type: 'semantic' as const, importance: 1.0 },
    ...
    { content: "COGXAI retrieval formula: score = recency * 0.5 + relevance * 3.0 + importance * 2.0 + vector_sim * 3.0 + graph_boost * 1.5", type: 'semantic' as const, importance: 0.9 },
    { content: "Venice AI provides private inference. COGXAI uses Venice for embedding and LLM calls", type: 'semantic' as const, importance: 0.8 },
    { content: "The COGXAI token will launch on Base. Contract address will be announced on cogxai.org", type: 'semantic' as const, importance: 1.0 },

    // Events
    { content: "CogxAI launched in March 2026 as an open-source persistent memory layer for AI agents", type: 'episodic' as const, importance: 0.9 },
    { content: "CogxAI supports three deployment modes: local with SQLite, self-hosted with Supabase, and hosted via Cortex API", type: 'episodic' as const, importance: 0.7 },
    { content: "FEiKU joined as marketing advisor, helped reposition CogxAI with Brain + Cortex naming", type: 'episodic' as const, importance: 0.7 },
    { content: "Benchmark hit 83.9/100 on March 4, 2026 after fixing knowledge seed scoring", type: 'episodic' as const, importance: 0.8 },
    { content: "Red Beard Ventures scheduled a VC call to discuss potential investment in COGXAI", type: 'episodic' as const, importance: 0.7 },

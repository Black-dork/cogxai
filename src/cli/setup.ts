...
function installMcpConfig(target: McpTarget, agentName: string, wallet: string, apiKey?: string, local?: boolean): { ok: boolean; error?: string } {
  ...
  try {
    ...
    config[target.wrapKey]['cogxai-memory'] = mcpEntry;
...
export async function runSetup(): Promise<void> {
  ...
  printInfo('We\'ll create an account on cogxai.org and get you an API key.');
  ...
  if (!existingKey) {
    ...
    wallet = await ask(rl, 'Base wallet address (Enter to skip): ');
    console.log('');
    ...
    try {
      const res = await fetch('https://cogxai.org/api/cortex/register', {
        method: 'POST',
      ...
      if (isCert) {
        ...
        printWarn(`Could not reach cogxai.org: ${msg}`);
      }
      printInfo('You can register manually later: npx cogxai register');
      apiKey = await ask(rl, 'Paste an API key (or Enter to skip): ');
    }
  ...
  if (mcpChoice !== 'skip') {
    ...
    for (const target of toInstall) {
      ...
      if (result.ok) {
        printSuccess(`Added cogxai-memory to ${target.label}`);
    ...
    printInfo('Skipped — run `npx cogxai mcp-install` anytime to set this up');
  }
  ...
  if (apiKey) {
    ...
    printWarn('No API key yet — run: npx cogxai register');
  }
  ...
  console.log(`  ${c.dim}Dashboard:${c.reset}  ${c.cyan}https://cogxai.org/explore${c.reset}`);
  console.log(`  ${c.dim}Docs:${c.reset}       ${c.cyan}https://cogxai.org/docs${c.reset}`);
  console.log(`  ${c.dim}Export:${c.reset}     ${c.cyan}npx cogxai export${c.reset}`);
  printDivider();
  console.log('');
...
export async function runMcpInstall(): Promise<void> {
  ...
  console.log(`  ${c.gray}Add CogxAI memory to your AI IDE.${c.reset}\n`);

...
- NEVER output URLs, links, or anything resembling a web address (no http://, no cogxai.org/path, no t.co/ links).
...
- The ONLY exception is basescan.org transaction links that YOU generate from on-chain data.`);
...
export async function generateImportanceScore(description: string): Promise<string> {
  const importancePrompt = process.env.COGXAI_IMPORTANCE_PROMPT ||

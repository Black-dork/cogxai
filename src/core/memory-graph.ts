...
//
// Extends CogxAI's memory system with explicit entity tracking.
// Instead of just memory→memory links, we now have:
//   Entity ←→ Memory (entity mentions)
//   Entity ←→ Entity (relationships)
//
// This enables:
//   - "Tell me about Alice" → find all memories mentioning Alice
//   - "How are X and Y related?" → traverse entity relationships
//   - Visual knowledge graph for cogxai.org
// ============================================================
...
export function extractEntitiesFromText(text: string): Array<{ name: string; type: EntityType }> {
  ...
  // Wallet addresses (Base hex)
  const wallets = text.match(/0x[0-9a-fA-F]{40}/g);
  if (wallets) {

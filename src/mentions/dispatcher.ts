...
import { getVestingInfo, getCAResponse, COGXAI_CA, getTokenStatus } from '../knowledge/tokenomics';
...
// Tweets/conversations to ignore completely (even if CogxAI is tagged).
// Add tweet IDs or conversation IDs here to block engagement.
const BLOCKED_TWEET_IDS = new Set([
  '2027286569233813567',  // mickeymantled thread - manual block
]);
...
Be direct and informative. If they ask about a specific aspect (like community allocation), focus on that.`;
...
async function handleWebSearch(
  ...
  try {
    ...
    const { content, citations } = await generateVeniceResponseWithSearch({
      ...
      systemPrompt:
        'You are CogxAI, an AI agent with persistent on-chain memory on Base. ' +
...
- Read the conversation thread carefully. If people are discussing OTHER projects, agents, or tokens — do NOT assume they are talking about you (CogxAI).
...
- If someone is clearly talking about a different project (not CogxAI), acknowledge that and don't insert yourself.`;
...
async function storeInteractionMemory(
  ...
  const containsAddress = /0x[0-9a-fA-F]{40}/.test(cleanText);

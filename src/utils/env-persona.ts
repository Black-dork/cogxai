 ...
 * All personality-revealing content lives in COGXAI_* env vars, not in source.
 */
...
function getInstructions(): Record<string, string> {
  if (!_instructions) {
    const raw = process.env.COGXAI_INSTRUCTIONS;
    if (raw) {
...
function getReplyPools(): Record<string, string[]> {
  if (!_replyPools) {
    const raw = process.env.COGXAI_REPLY_POOLS;
    if (raw) {
